...
export class Chart3ConfigComponent implements OnInit{
  ...
  moduleNamesTooltip: string = 'Select the module to which the user will be redirected when the widget is clicked.';
  FormNames: any;
    ...
    this.api
      ...
      .then((result: any) => {
        if (result && result.metadata) {
          ...
          .filter((field: any) => {
            // Filter out fields with type "heading" or with an empty placeholder
            return field.type !== "heading" && field.type !== 'Empty Placeholder' && field.type !=='button' && field.type !=='table' && field.type !=='radio' && field.type !== 'checkbox' && field.type !=='datetime-local' && field.type !== 'html code' && field.type !=='file' && field.type !=='range' && field.type !=='color' && field.type !=='password' && field.type !=='sub heading';
          })
          .map((field: any) => {
            console.log('field check', field);
            return {
