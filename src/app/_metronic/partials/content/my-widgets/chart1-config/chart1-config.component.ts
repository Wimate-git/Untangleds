  ...
  if (parsedChartConfig.length > 0) {
    parsedChartConfig.forEach((configItem, index) => {
      ...
      const arrayParameter = Array.isArray(configItem.parameterName)
        ...
        console.log('arrayParameter checking from readback',arrayParameter)
...
this.dynamicDateParamMap.set(index,dateFieldsList)
  ...
  selectFormParams(event: any, index: number) {
    if (event && event[0] && event[0].data) {
      ...
      // // Set the parameterName field to an empty array
      // fieldGroup.get('parameterName')?.setValue([]);
...
preventNegativeValue(event: Event) {
  ...
  if (+input.value < 0) {
    input.value = '0';
    this.createChart.get('CustomValueLabelfontSize')?.setValue(0);
  }
}
