...
export class Chart2ConfigComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.createChart.get('toggleCheck')?.valueChanges.subscribe((isChecked) => {
      console.log('is checked verify',isChecked)
      if (isChecked) {
...
async moduleSelection(event: any): Promise<void> {
  ...
  if (selectedValue === 'Summary Dashboard') {
    ...
    this.filteredSelectTypeSummary = this.SelectTypeSummary.filter(
      (      item: { value: string; }) => item.value !== 'Modal' && item.value !== 'drill down'
    );
...
CustomLineColorTooltip: string = 'Defines the color of the line in the line chart. The selected color will be applied to the plotted line.';
