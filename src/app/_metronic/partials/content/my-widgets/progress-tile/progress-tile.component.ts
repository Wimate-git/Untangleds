...
@Component({
  ...
  styleUrl: './progress-tile.component.scss'
})
export class ProgressTileComponent implements OnInit{
...
// Default unchecked
  ...
  addControls(event: any, _type: string) {
    ...
    // Restrict maximum number of configurations to 2
    if (noOfParams > 2) {
      this.toast.open("Maximum of 2 parameters allowed", "Limit Exceeded", {
        duration: 5000,
        horizontalPosition: 'right',
        verticalPosition: 'top',
      });
...
// alert('cloned tile')
  ...
  openProgressTileModal(tile: any, index: number) {
