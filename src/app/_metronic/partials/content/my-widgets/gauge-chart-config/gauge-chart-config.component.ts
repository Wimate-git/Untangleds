...
@Component({
  selector: 'app-gauge-chart-config',

  templateUrl: './gauge-chart-config.component.html',
  styleUrl: './gauge-chart-config.component.scss'
})
export class GaugeChartConfigComponent {
  createChart:FormGroup
    ...
    //   console.log('event checking from form array control',event)
    ...
    //     //   verticalPosition: 'top',
    //     // });
    //     event.target.value = 2;  // Reset to 1 if a value greater than 1 is entered
    //   }
    ...
    onCombinedAddFieldsChange(event: any): void {
      ...
      // Validate the value is greater than 0 and less than or equal to 2
      if (value <= 0) {
        this.toast.open("The parameter must be greater than 0.", "Check again", {
          duration: 5000,
          horizontalPosition: 'right',
        ...
        event.target.value = 2;  // Reset to 2 if a value greater than 2 is entered
      }
    
      // Call addControls with updated value
      this.addControls(event, 'html');
    }
    ...
    addControls(event: any, _type: string) {
      ...
      if (_type === 'html' && event && event.target) {
        if (event.target.value >= 0) {
          noOfParams = event.target.value; // Directly use the value from the input field
        } else {
      ...
      // Ensure the form array has no more than 2 form groups
      if (noOfParams > 0) {
        // Clear the existing controls if it is a FormArray
        const allFieldsArray = this.createChart.get('all_fields') as FormArray;
        allFieldsArray.clear(); // This will clear the FormArray
    
        // Add new controls based on the new parameter count
      ...
      // Update noOfParams for future use in addTile (if needed)
  ...
  opengaugeChartModal(tile: any, index: number): void {
...
setTimeout(() => {
  ...
  repopulate_fields(getValues: any): FormArray {
    console.log('getValues check from repopulate_fields', getValues);
    ...
    // Ensure `parsedChartConfig` is populated
    if (parsedChartConfig.length > 0) {
    ...
    // Ensure that the form control updates are reflected in the view
    this.cdr.detectChanges();
  ...
  toggleCheckbox(theme: any): void {
    ...
    defaultHighchartsOptionsJson: any = {
      chart: {
        type: 'solidgauge'  // Set the chart type to solid gauge
      },
      title: null,  // No title
      pane: {
        startAngle: -90,  // Starting angle of the gauge
        endAngle: 90,     // Ending angle of the gauge
        background: {
          outerRadius: '109%',
          innerRadius: '95%',
          shape: 'arc',
      ...
      yAxis: {
        ...
        plotBands: [{
          from: 0,
          to: 50,
          color: '#55BF3B' // Green color for range 0-50
        }, {
          from: 50,
          to: 80,
          color: '#DDDF0D' // Yellow color for range 50-80
        }, {
          from: 80,
          to: 100,
          color: '#DF5353' // Red color for range 80-100
        }]
      },
      series: [{
        name: 'Speed',
        data: [80],  // Initial value of the gauge
        dataLabels: {
          format: '{y} km/h',
          color: '#000',
          fontSize: '30px',
  ...
  validateAndUpdate() {
    ...
    Object.values(this.createChart.controls).forEach(control => {
      if (control instanceof FormControl) {
        // If required field is empty, mark the form as invalid
    ...
    // Check overall form validity (including all_fields FormArray)
    const allFieldsValid = this.createChart.get('all_fields')?.valid;
  
    console.log('Check form array validity:', allFieldsValid);
