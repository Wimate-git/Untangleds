...
export class ProjectTemplateComponent implements OnInit, AfterViewInit, OnDestroy {
...
addValues() {
  ...
  previewIcon(event: any) {
    ...
    const selectedIcon = this.iconsList.find((packet: any) => {
        return packet.value === this.projectForm.get('iconSelect')?.value;
    });
  ...
  delete(P1: any) {
    ...
    const body = {
      PK: this.client + '#' + 'projectTemplate#' + P1 + '#main',
      SK: 1,
    };
  ...
  onSubmit(event: Event) {
    ...
    const successAlert: SweetAlertOptions = {
      icon: 'success',
      title: 'Success!',
      text: this.match ? 'Project Template updated successfully!' : 'Project Template created successfully!',
    };
    ...
    const createFn = async () => {
      ...
      const body = {
        PK: this.client + '#' + 'projectTemplate#' + this.projectForm.value.labelID + '#main',
        SK: 1,
      ...
      try {
        ...
        await this.createLookuppermission(this.projectgroupItem, 1, this.client + '#projectTemplate#lookup');
    ...
    const updateFn = () => {
      ...
      const body = {
        PK: this.client + '#' + 'projectTemplate#' + matchingRecord.P1 + '#main',
        SK: 1,
        metadata: JSON.stringify(formValue)
      };
  ...
  formgroupLookupData(sk: number) {
    ...
    return new Promise((resolve, reject) => {
      this.api.GetMaster(this.client + '#' + 'projectTemplate#lookup', sk)
        .then(response => {
  ...
  async forms(formValue:any){

    this.api.GetMaster(this.client+'#dynamic_form#'+formValue+'#main',1).then((formres:any)=>{
  ...
  async updatedreamboardlookup(sk: any, id: any, type: any, item: any) {
    ...
    const tempClient = this.client + '#projectTemplate' + "#lookup";
