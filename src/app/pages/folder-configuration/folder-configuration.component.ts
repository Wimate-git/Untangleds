...
export class FolderConfigurationComponent implements OnInit, AfterViewInit, OnDestroy {
 ...
 selectedColor: string = ''
  ...
  previewIcon(event: any) {
    ...
    const selectedIcon = this.iconsList.find((packet: any) => {
        return packet.value === this.folderForm.get('iconSelect')?.value;
    });
  ...
  delete(P1: any) {
    ...
    this.api.DeleteMaster(body).then(async (res: any) => {
      ...
      this.formgroupItem = {
        ...
        P4: this.deleteResponse.color,
        P5: JSON.stringify(this.deleteResponse.iconObject),
  ...
  edit(P1: any) {
    ...
    this.api.GetMaster(this.client + '#folder#' + P1 + '#main', 1).then((res: any) => {
      ...
      if (res && res !== undefined) {
        ...
        if (this.data_temp) {
          ...
          this.folderForm = this.fb.group({
            ...
            remarks:[this.data_temp[0].remarks],
  ...
  onSubmit(event: Event) {
    ...
    const formgroupId = this.folderForm.value.labelID;
    ...
    const successAlert: SweetAlertOptions = {
      icon: 'success',
      title: 'Success!',
      text: this.match ? 'Project Group updated successfully!' : 'Project Group created successfully!',
    };
    ...
    const createFn = async () => {
      ...
      if (!this.folderForm.valid) {
        console.log('Form is not valid:', this.folderForm.errors);
        return;
      }
      this.isLoading = true;
      ...
      this.folderForm.value.iconObject = this.previewObjDisplay

      console.log("CREATE FORMGROUP:",formValue)

      this.folderForm.value.createdUser = this.users

      // if(this.selectedImage){
      //   this.folderForm.value.selectedImage = this.selectedImage
      // }

      const body = {
        PK: this.client + '#' + 'folder#' + this.folderForm.value.labelID + '#main',
        SK: 1,
      ...
      try {
        ...
        this.formgroupItem = {
          ...
          P8: this.folderForm.value.updatedTime
        };

        console.log("LOOK UP DREAMBOARD DATA:", this.formgroupItem);
        await this.createLookupdreamboard(this.formgroupItem, 1, this.client + '#folder#lookup');
    ...
    const updateFn = () => {
      ...
      this.folderForm.value.selectedImage = this.data_temp[0].selectedImage
      
      }

      this.folderForm.value.updatedUser = this.users

      const body = {
        PK: this.client + '#' + 'folder#' + matchingRecord.P1 + '#main',
        SK: 1,
        metadata: JSON.stringify(formValue)
      };
      ...
      this.api.UpdateMaster(body).then(async (res: any) => {
        ...
        this.formgroupItem = {
          labelID: this.updateResponse.labelID,
          description: this.updateResponse.description,
  ...
  formgroupLookupData(sk: number) {
    ...
    return new Promise((resolve, reject) => {
      this.api.GetMaster(this.client + '#' + 'folder#lookup', sk)
        .then(response => {
  ...
  async fetchprojectTemplatelookup(sk: any) {
    try {
      ...
      if (response && response.options) {
          ...
          if (Array.isArray(data)) {
            for (let index = 0; index < data.length; index++) {
              ...
              if (element !== null && element !== undefined) {
                ...
                this.formList.push({ P1, P2, P3, P4 }); // Push an array containing P1, P2, and P3 values
            ...
            this.formList.sort((a: any, b: any) => {
              return b.P4 - a.P4; // Compare P5 values in descending order
            });
            console.log("Lookup sorting", this.formList);
            // Continue fetching recursively
        ...
        this.formList = this.formList.map((item: any) => item.P1)
  ...
  async updatedreamboardlookup(sk: any, id: any, type: any, item: any) {
    ...
    const tempClient = this.client + '#folder' + "#lookup";
